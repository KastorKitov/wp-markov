...
- **`Запитване` CTA URL is filterable** via `kastor_machines_inquiry_url`. Default points to `/kontakti/?machine=<title>&machine_id=<id>` so the contact form can pre-fill. Override the path or query shape from a snippet without touching the plugin.
